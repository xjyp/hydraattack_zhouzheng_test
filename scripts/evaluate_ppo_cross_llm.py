...
"""
PPO跨LLM泛化评估脚本
加载在Qwen3-8B作为judge情况下训练的PPO模型，测试攻击Qwen3-0.6B的效果
"""
...
def load_ppo_agent(model_path: str, logger: HydraLogger = None) -> PPOAgent:
    """加载训练好的PPO智能体"""
    ...
    # 从策略网络的第一个线性层的权重形状推断状态维度和隐藏层维度
    if 'policy_network_state_dict' in checkpoint:
        state_dict = checkpoint['policy_network_state_dict']
    ...
    if first_layer_weight is not None:
        ...
        hidden_dim = first_layer_weight.shape[0]  # 输出维度（隐藏层维度）
    else:
        state_dim = 512  # 默认值
        hidden_dim = 512  # 默认值
    ...
    if logger:
        ...
        logger.logger.info(f"   推断的隐藏层维度: {hidden_dim}")
    ...
    agent = PPOAgent(
        ...
        hidden_dim=hidden_dim,  # 使用推断的隐藏层维度
        learning_rate=3e-4,
        gamma=0.99,
        gae_lambda=0.95,
        clip_range=0.2,
    ...
    if logger:
        logger.logger.info(f"✅ 成功加载PPO模型: {model_path}")
        logger.logger.info(f"   状态维度: {state_dim}")
        logger.logger.info(f"   动作空间大小: {total_actions}")
        logger.logger.info(f"   隐藏层维度: {hidden_dim}")
...
def evaluate_cross_llm_performance(agent, test_examples: List[PairwiseExample], 
    ...
    if logger:
        logger.logger.info(f"📊 跨LLM泛化评估结果:")
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description="PPO跨LLM泛化评估")
    ...
    parser.add_argument("--ppo_model_path", type=str, required=True, help="PPO模型路径")
    parser.add_argument("--max_queries", type=int, default=10, help="最大查询次数")
    parser.add_argument("--output_dir", type=str, default="./cross_llm_results", help="结果输出目录")
    ...
    logger.logger.info("🚀 开始PPO跨LLM泛化评估")
    ...
    logger.logger.info(f"PPO模型: {args.ppo_model_path}")
    ...
    # 加载PPO智能体
    try:
        agent = load_ppo_agent(args.ppo_model_path, logger)
        logger.logger.info("✅ PPO智能体加载成功")
    except Exception as e:
        logger.logger.error(f"❌ PPO智能体加载失败: {e}")
        return
    ...
    results_file = os.path.join(args.output_dir, f"ppo_cross_llm_evaluation_{args.benchmark}_{timestamp}.json")

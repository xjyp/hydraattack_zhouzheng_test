...
from rl.agent import GRPOAgent
from rl.trainer import RLTrainer, evaluate_on_unseen_data
from data_types import RLConfig
...
def load_benchmark_data(benchmark: str, max_samples = 100, random_seed: int = 42, validation_ratio: float = 0.1) -> tuple[List[PairwiseExample], List[PairwiseExample]]:
    ...
    if max_samples != "full" and max_samples is not None:
        try:
            ...
            raise ValueError(f"max_samples无法转换为整数: {max_samples}")
...
def main():
    ...
    parser.add_argument("--validation_ratio", type=float, default=0.1, help="验证集比例")
    ...
    parser.add_argument("--clip_range", type=float, default=0.2, help="GRPO裁剪范围")
    ...
    parser.add_argument("--max_grad_norm", type=float, default=0.5, help="梯度裁剪最大值")
    parser.add_argument("--n_epochs", type=int, default=10, help="每次更新的训练轮数")
    parser.add_argument("--group_size", type=int, default=4, help="组大小")
    parser.add_argument("--relative_weight", type=float, default=0.3, help="相对权重")
    parser.add_argument("--memory_size", type=int, default=10000, help="经验回放内存大小")
    parser.add_argument("--target_update_freq", type=int, default=100, help="目标网络更新频率")
    ...
    # 设置所有随机种子（包括PyTorch和CUDA）
    set_seed(args.random_seed, deterministic=True)
    ...
    logger.logger.info("🚀 开始训练快速RL攻击智能体（GRPO算法，支持数据分割）")
    ...
    logger.logger.info(f"数据分割: 训练{args.train_ratio:.1%} / 测试{args.test_ratio:.1%}")
    ...
    if validation_examples:
        logger.logger.info(f"✅ 分割了 {len(validation_examples)} 个验证样本")
    ...
    logger.logger.info(f"✅ 创建GRPO智能体: 状态维度={state_dim}, 动作维度={action_dim}")
    logger.logger.info(f"   GRPO参数: 学习率={args.learning_rate}, 批次大小={args.batch_size}, 隐藏层维度={args.hidden_dim}")
    logger.logger.info(f"   网络参数: γ={args.gamma}, GAE_λ={args.gae_lambda}, 裁剪范围={args.clip_range}")
    logger.logger.info(f"   损失参数: 价值损失系数={args.value_loss_coef}, 熵系数={args.entropy_coef}, 梯度裁剪={args.max_grad_norm}")
    logger.logger.info(f"   训练参数: 训练轮数={args.n_epochs}")
    logger.logger.info(f"   GRPO特有参数: 组大小={args.group_size}, 相对权重={args.relative_weight}, 内存大小={args.memory_size}, 目标更新频率={args.target_update_freq}")
    ...
    # 保存模型 - 文件名包含数据集信息和算法信息，新模型会覆盖旧模型
    model_path = os.path.join(args.model_dir, f"fast_rl_attacker_grpo_{args.benchmark}.pth")
    ...
    mapping_path = os.path.join(args.model_dir, f"action_mapping_grpo_{args.benchmark}.json")
    with open(mapping_path, 'w') as f:
        json.dump(action_mapping, f, indent=2)
    logger.logger.info(f"💾 动作映射已保存到: {mapping_path}")
    
    # 保存训练配置和结果 - 文件名包含数据集信息和算法信息，新模型会覆盖旧模型
    config_dict = {
        'algorithm': 'GRPO',
    ...
    config_path = os.path.join(args.model_dir, f"training_config_grpo_{args.benchmark}.json")

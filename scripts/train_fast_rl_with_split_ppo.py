...
def main():
    ...
    logger.logger.info("🚀 开始训练快速RL攻击智能体（PPO算法，支持数据分割）")
    ...
    logger.logger.info(f"✅ 创建PPO智能体: 状态维度={state_dim}, 动作维度={action_dim}")
    logger.logger.info(f"   PPO参数: 学习率={args.learning_rate}, 批次大小={args.batch_size}, 隐藏层维度={args.hidden_dim}")
    ...
    model_path = os.path.join(args.model_dir, f"fast_rl_attacker_ppo_{args.benchmark}.pth")
    ...
    mapping_path = os.path.join(args.model_dir, f"action_mapping_ppo_{args.benchmark}.json")
    ...
    config_path = os.path.join(args.model_dir, f"training_config_ppo_{args.benchmark}.json")
